...
# =============================
# Enhanced Multi-VM Manager (Fixed Start VM)
# =============================
...
========================================================================
 ...
 |_|  |_|\____/|_|   |_____|_| \_|\_____|____/ \____/  |_|  /_____|
                    ...
                    POWERED BY HOPINGBOYZ | FIXED BY GROK
========================================================================
EOF
    echo
}
...
load_vm_config() {
    ...
    if [[ -f "$config_file" ]]; then
        ...
        unset DISK_SIZE MEMORY CPUS SSH_PORT GUI_MODE PORT_FORWARDS IMG_FILE SEED_FILE CREATED SWAP_SIZE
...
create_new_vm() {
    ...
    while true; do
        read -p "$(print_status "INPUT" "Swap size (default: 2G, 0G to disable): ")" SWAP_SIZE
        SWAP_SIZE="${SWAP_SIZE:-2G}"
        if validate_input "size" "$SWAP_SIZE"; then
            break
        fi
    done
    ...
    while true; do
        ...
        if validate_input "number" "$MEMORY"; then
            MEMORY="${MEMORY}M"  # Format for QEMU
            break
        fi
    done
...
# Function to setup VM image (FIX: resize logic, add swap/packages)
setup_vm_image() {
    ...
    # FIXED Resize the disk image
    print_status "INFO" "Resizing image to $DISK_SIZE..."
    if ! qemu-img resize -f qcow2 "$IMG_FILE" "$DISK_SIZE"; then
        print_status "WARN" "Resize failed, creating new image..."
        rm -f "$IMG_FILE"
        qemu-img create -f qcow2 "$IMG_FILE" "$DISK_SIZE"
    fi

    # cloud-init configuration (ADD: swap, openssh, growpart)
    cat > user-data <<EOF
#cloud-config
hostname: $HOSTNAME
...
runcmd:
  ...
  - |
    if [ "\$(echo $SWAP_SIZE | sed 's/G//')" != "0" ]; then
...
# Function to start a VM (FIX: KVM check, port forwards single netdev, graceful trap)
start_vm() {
    local vm_name=$1
    ...
    if load_vm_config "$vm_name"; then
        ...
        print_status "INFO" "Password: [HIDDEN - use $PASSWORD]"
        ...
        if [ -e /dev/kvm ] && groups | grep -q kvm; then
            ...
            print_status "WARN" "KVM unavailable (add user to kvm group?), using TCG."
        fi
        
        # FIXED Port forwards: single netdev with multiple hostfwd
        ...
        if [[ -n "$PORT_FORWARDS" ]]; then
            ...
            print_status "INFO" "Added port forwards: $PORT_FORWARDS"
        fi
        
        # Base QEMU command (FIX: use string for simplicity, add cache)
        local QEMU_CMD="qemu-system-x86_64 $ACCEL_FLAG -m $MEMORY -smp $CPUS \
        ...
        trap 'print_status "INFO" "Shutting down VM gracefully..."; pkill -f "qemu-system-x86_64.*$IMG_FILE" || true; trap - SIGINT SIGTERM EXIT; exit' SIGINT SIGTERM
        
        print_status "INFO" "Starting QEMU... (Press Ctrl+C to stop)"
...
delete_vm() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        if load_vm_config "$vm_name"; then
            stop_vm "$vm_name"  # FIXED: Stop first
...
# Function to check if VM is running (FIX: grep by IMG_FILE)
is_vm_running() {
    local vm_name=$1
    if load_vm_config "$vm_name" &>/dev/null; then
        pgrep -f "qemu-system-x86_64.*$IMG_FILE" >/dev/null
    else
        return 1
    fi
}

# Function to stop a running VM (FIX: use IMG_FILE grep)
stop_vm() {
    local vm_name=$1
...
# [SISA FUNGSI: edit_vm_config, resize_vm_disk, show_vm_performance, main_menu - SAMA SEPERTI SKRIP ANDA, TIDAK DIUBAH]
# ... (copy dari skrip Anda untuk bagian ini, karena sudah OK)
